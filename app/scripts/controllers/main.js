...
angular.module('hnhtApp')
    .controller('MainCtrl', function ($scope, $location, $timeout, items, latestThread) {

  var getId = function() {
    latestThread.get({}, function(data){
      var sigil = data.results[0].item._id;
      items.get({
        'limit': limit,
        'start': start,
        'filter[fields][parent_sigid]': sigil,
        'q': q.join(' ')
      }, function (data){
